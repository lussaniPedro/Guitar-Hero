...
#define COLUMNS 5
#define TIME_BAR 50
#define PAUSE gh_getch();
#define SKIP_PAUSE() if(gh_getch() == ' '){ CLS printf("Tutorial skipped!"); Sleep(900); return; }
...
void tutorial(); // Game tutorial
void endGame(int gameScore, bool newRecord); // Finish the game
void clearBuffer(); // (fflush(stfin))
...
char controls[6] = {"ASJKL"}; // Guitar controls
...
int selection(char **options, int size){
    ...
    while(1){
        ...
        if(key == (int)'g' || key == (int)'G') return key;

        switch(key){
            case GH_UP:    pos = (pos <= 0) ? size - 1 : pos - 1; break;
            case GH_DOWN:  pos = (pos >= size - 1) ? 0 : pos + 1; break;
            case GH_ENTER: return pos;
            case GH_ESC:   return GH_ESC;
        }
    }
}
...
void tellemcontrols(){
    ...
    printf("\n___________________\n Menu ops -> Keys:\n\n");
    printf(" GH_ENTER = Open menu \n");
    printf("GH_ESC = Exit the menu\n\n");
    printf("       %s↑%s  %s↓%s        \n", GREEN, RESET, RED, RESET);
    printf("      GH_UP GH_DOWN\n");
    printf("       %s→%s  %s←%s        \n", BLUE, RESET, ORANGE, RESET);
    printf("    RIGHT GH_LEFT\n");
    printf("___________________\n\n\n");
    printf(RESET);
...
void play(int dif){
    ...
    while(1){
        ...
        if(count < ticks){
            int barProgress = (count * TIME_BAR) / ticks;
            ...
            for(int i = barProgress; i < TIME_BAR; i++){
                printf(" ");
            }
        ...
        int check = 0;
        if(gh_kbhit()){ // Check if a key is pressed
            char pressed = gh_getch();
...
void saveGame(){
    ...
    FILE *gameFile = fopen("output/GuitarHero.txt", "w");
    if(gameFile == NULL){
        errorMessage(-2);
        SPAUSE
    }
...
void loadGame(){
    ...
    FILE *gameFile = fopen("output/GuitarHero.txt", "r");
    if(gameFile){
        i = 0;
        sep = 0;
